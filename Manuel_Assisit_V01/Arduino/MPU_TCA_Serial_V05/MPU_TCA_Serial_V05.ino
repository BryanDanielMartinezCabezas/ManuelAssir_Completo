...
String clasificarMovimiento(uint8_t i) {
  ...
  // PUENTE: solo índice y pulgar.
  // Si viene de ultra y cae al puente entre ultra y normal,
  // no baja a normal, va directo a reposo.
  if ((i == INDICE || i == PULGAR) && estadoConfirmado[i] == "ultra") {
    if (intensidad < ULTRA_OFF[i] && intensidad >= NORMAL_ON[i]) {
      return "reposo";
    }
  }
  ...
  // Solo indice y pulgar tienen normal.
  if ((i == INDICE || i == PULGAR) && intensidad >= NORMAL_ON[i]) {
    return "normal";
  }
...
void confirmarEstado(uint8_t i, String nuevoEstado) {
  ...
  if (contadorEstable[i] >= FRAMES_CONFIRMACION) {
    ...
    // Solo menique, anular y medio tienen autoreposo.
    // Indice y pulgar conservan su estado.
    if (i != INDICE && i != PULGAR && estadoAnterior == "reposo") {
...
void resetearDedosBloqueados(int activo) {
  for (uint8_t i = 0; i < NUM_SENSORES; i++) {
    ...
    // Combinación permitida: pulgar + medio pueden estar activos juntos.
    if (activo == PULGAR && i == MEDIO) continue;
    if (activo == MEDIO  && i == PULGAR) continue;
...
void actualizarDedoActivo(float dt) {
  ...
  // Combinación permitida pulgar + medio.
  if (dedoActivo == PULGAR) {
    actualizarDedo(MEDIO, dt);
  }
  else if (dedoActivo == MEDIO) {
    actualizarDedo(PULGAR, dt);
  }
